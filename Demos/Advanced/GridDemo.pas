...
begin
  ...
  begin
    VST5.Header.Columns[0].Options := VST5.Header.Columns[1].Options - [{$IF CompilerVersion >= 20}TVTColumnOption.{$IFEND}coVisible]; //test:
...
begin
  if GridLineCheckBox.Checked then
    VST5.TreeOptions.PaintOptions := VST5.TreeOptions.PaintOptions + [{$IF CompilerVersion >= 20}TVTPaintOption.{$IFEND}toShowHorzGridLines, {$IF CompilerVersion >= 20}TVTPaintOption.{$IFEND}toShowVertGridLines]
  else
    VST5.TreeOptions.PaintOptions := VST5.TreeOptions.PaintOptions - [{$IF CompilerVersion >= 20}TVTPaintOption.{$IFEND}toShowHorzGridLines, {$IF CompilerVersion >= 20}TVTPaintOption.{$IFEND}toShowVertGridLines];
end;
...
begin
  if Column = 0 then
    ...
    begin
      ...
      if {$IF CompilerVersion >= 20}TVTPaintOption.{$IFEND}toShowVertGridLines in VST5.TreeOptions.PaintOptions then
        Inc(CellRect.Right);
      if {$IF CompilerVersion >= 20}TVTPaintOption.{$IFEND}toShowHorzGridLines in VST5.TreeOptions.PaintOptions then
        Inc(CellRect.Bottom);
...
begin
  if AutoSpanCheckBox.Checked then
    VST5.TreeOptions.AutoOptions := VST5.TreeOptions.AutoOptions + [{$IF CompilerVersion >= 20}TVTAutoOption.{$IFEND}toAutoSpanColumns]
  else
    VST5.TreeOptions.AutoOptions := VST5.TreeOptions.AutoOptions - [{$IF CompilerVersion >= 20}TVTAutoOption.{$IFEND}toAutoSpanColumns];
end;
